...
from courier import MainSG, dialog, fetch

logging.basicConfig(level=logging.INFO)

env = environ.Env()
env.read_env(Path(__file__).resolve().parent.parent.parent / ".env")
...
async def start(m: Message, dialog_manager: DialogManager):
    await m.answer("Привет, я помогаяю курьерам с доставкой."
